...
const InfoPlikuDoRysowania* LaczeDoGNUPlota::ZnajdzNazwePliku (const char *sNazwaPliku, TypDostepuDoZasobu Dostep) const
{
    const std::list<InfoPlikuDoRysowania>  &_ListaPlikow =
                     (Dostep == DZ_Globalny) ? _InfoPlikow_Glb : _InfoPlikow_Lok;
    for ( const InfoPlikuDoRysowania  &InfoPliku :  _ListaPlikow) {
...
bool LaczeDoGNUPlota::UsunNazwePliku(  const InfoPlikuDoRysowania*  wInfoPliku, TypDostepuDoZasobu Dostep)
{
return UsunNazwePliku(wInfoPliku->WezNazwePliku(),Dostep);
}

bool LaczeDoGNUPlota::UsunNazwePliku(const char *sNazwaPliku, TypDostepuDoZasobu   Dostep)
{
return UsunNazwePliku(std::string(sNazwaPliku),Dostep);
}

bool LaczeDoGNUPlota::UsunNazwePliku( const std::string &NazwaPliku, TypDostepuDoZasobu Dostep)
{
    unsigned int IloscUsunietych = 0;
...
InfoPlikuDoRysowania&  LaczeDoGNUPlota::DodajNazwePliku( const char         *NazwaPliku,
                                                         bool                SprawdzIstnienie,
                                                         TypDostepuDoZasobu  Dostep)
{
    if (SprawdzIstnienie && !CzyJestPlik(NazwaPliku)) {
...
InfoPlikuDoRysowania& LaczeDoGNUPlota::DodajNazwePliku_PunktyRoznejWielkosci( const char         *NazwaPliku,
                                                                              bool                SprawdzIstnienie,
                                                                              TypDostepuDoZasobu  Dostep )
{
    if (SprawdzIstnienie && !CzyJestPlik(NazwaPliku)) {
...
InfoPlikuDoRysowania& LaczeDoGNUPlota::DodajNazwePliku_Lamana( const char         *NazwaPliku,
                                                               bool                SprawdzIstnienie,
                                                               TypDostepuDoZasobu  Dostep )
{
    return DodajNazwePliku(NazwaPliku,SprawdzIstnienie,Dostep).ZmienSposobRys(SR_Ciagly);
}

InfoPlikuDoRysowania& LaczeDoGNUPlota::DodajNazwePliku_JednakowePunkty( const char         *NazwaPliku,
                                                                        bool                SprawdzIstnienie,
                                                                        TypDostepuDoZasobu  Dostep )
{
    return DodajNazwePliku(NazwaPliku,SprawdzIstnienie,Dostep).ZmienSposobRys(SR_Punktowy);
}
...
bool LaczeDoGNUPlota::UtworzProcesPotomny()
{
    ...
    if (pipe(indesk) == -1 || pipe(outdesk) == -1) {
        KomunikatBledu("### Niemozliwe utworzenie kanalow komunikacyjnych do podprocesow.");
        exit(1);
    }
    ...
    switch (fork()) {
      ...
      case  0:
       // Start potomka
        ...
        if (close(STDOUT) == -1) {
            KomunikatBledu("### Blad zamkniecia kanalu standardowego kanalu wyjsciowego.");
            exit(1);
        }
        if (dup(indesk[STDOUT]) == -1) {
            KomunikatBledu("### Blad duplikacji kanalu wyjsciowego.");
                             ...
                             char const                                **wwSep )
{
    ...
    for (; IterNazw != IterKonca; ++IterNazw ) {
        ...
        switch (IterNazw->WezSposobRys()) {
          case SR_Ciagly:
              TStrmWy << "w l " << " lt " << IterNazw->WezStyl() << " lw " << IterNazw->WezSzerokosc() << " lc " << IterNazw->WezKolor();
              break;

          case SR_Punktowy:
	          TStrmWy << "w p " << " pt " << IterNazw->WezStyl() << " ps " << IterNazw->WezSzerokosc() << " lc " << IterNazw->WezKolor();
	          break;
...
class AtrybutyRysowania {
  ...
  public:
    AtrybutyRysowania(){  _Styl = _Rozmiar = _Kolor = 0; }
    ...
    AtrybutyRysowania& ZmienRozmiar(int Rozmiar) { _Rozmiar = Rozmiar; return *this;}
 };
...
class Wektor2f {
  public:
      ...
      Wektor2f& operator += (const Wektor2f& Skl) { x += Skl.x;  y += Skl.y;  return *this; }

      Wektor2f  operator + (Wektor2f Skl2) const { return Skl2 += *this; }

      Wektor2f& operator *= (float Arg) { x *= Arg;  y *= Arg;  return *this; }

      Wektor2f  operator * (float Arg) const { return Wektor2f(*this) *= Arg; }
...
class InfoPunktyDoRysowania {
  private:
    ...
    Wektor2f  WyznaczWspolrzEtykiety() const
    {
        return _PrzesEtykiety_WzgRozmPunktu * _Atryb.WezRozmiar() + _PrzesEtykiety_WzgWspoPunktu + WezWspolrz();
    }
   ...
   public:
    ...
    const Wektor2f&  WezWspolrzEtykiety() const {  return _WspolrzEtykiety; }

    bool operator < (const InfoPunktyDoRysowania& Info) const {  return _Atryb < Info._Atryb; }
    ...
    InfoPunktyDoRysowania& ZmienPrzesEtykiety_WzgWspoPunktu( float   x, float   y);

    InfoPunktyDoRysowania& ZmienPrzesEtykiety_WzgRozmPunktu( float   x, float   y);
 };

Wektor2f InfoPunktyDoRysowania::_PrzesEtykiety_WzgWspoPunktu(0.5,0.5);
Wektor2f InfoPunktyDoRysowania::_PrzesEtykiety_WzgRozmPunktu(0.26,0.26);

InfoPunktyDoRysowania& InfoPunktyDoRysowania::ZmienPrzesEtykiety_WzgWspoPunktu( float   x, float   y)
{
  _PrzesEtykiety_WzgWspoPunktu.Zmien(x,y);   return *this;
}

InfoPunktyDoRysowania& InfoPunktyDoRysowania::ZmienPrzesEtykiety_WzgRozmPunktu( float   x, float   y)
{
  _PrzesEtykiety_WzgRozmPunktu.Zmien(x,y);  return *this;
}
