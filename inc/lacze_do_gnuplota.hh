...
class InfoPlikuDoRysowania {
  ...
  public:
    ...
    InfoPlikuDoRysowania& ZmienSposobRys (TypSposobuRysowania  SposobRys) { _SposobRys = SposobRys;  return *this; }

    int WezSzerokosc() const { return _Szerokosc; }

    InfoPlikuDoRysowania& ZmienSzerokosc (int Szerokosc) { _Szerokosc = Szerokosc;  return *this; }

    int WezKolor() const { return _Kolor; }

    InfoPlikuDoRysowania& ZmienKolor (int Kolor) { _Kolor = Kolor;  return *this; }

    int WezStyl() const { return _Styl; }

    InfoPlikuDoRysowania& ZmienStyl (int Styl) { _Styl = Styl;  return *this; }

    const std::string& WezEtykiete() const { return _Etykieta; }

    InfoPlikuDoRysowania& ZmienEtykiete (const char* Etykieta) { _Etykieta = Etykieta;  return *this; }
  };
...
class LaczeDoGNUPlota {
 protected:
  ...
  public:
    ...
    bool UsunNazwePliku( const char          *sNazwaPliku, TypDostepuDoZasobu   Dostep = DZ_Lokalny);

    bool UsunNazwePliku( const std::string   &NazwaPliku, TypDostepuDoZasobu   Dostep = DZ_Lokalny);

    bool DopiszRysowanieZPlikow( std::string &Polecenie, char const **Sep );

    bool DopiszRysowanieZRoznychPunktow( std::string   &Polecenie, char const    *Sep);
  ...
  inline
  bool LaczeDoGNUPlota::DopiszPlikiDoPoleceniaRysowania( std::string &, char const  **)
  { return false; }
